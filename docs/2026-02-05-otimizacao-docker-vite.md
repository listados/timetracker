...
```
docker compose up -d
├── timetracking-mysql   (porta 3306) — MySQL 8.0 com health check
├── timetracking-app     (porta 9000) — PHP-FPM + OPcache + caches Laravel
├── timetracking-vite    (porta 5173) — Vite dev server com HMR
└── timetracking-nginx   (porta 8080) — Proxy reverso
```
